...
def gold_medal_form(team1, team2):
    ...
    with st.form(key="gold_medal_form"):
        ...
        with col1:
            ...
            team2_names = " + ".join(get_player_name(pid) for pid in team2)
            st.write(f"{team1_names} vs. {team2_names}")
            score1_game1 = st.number_input(label=f"Score for {team1_names} (Game 1)", key="score_gold_1_game1", min_value=0)
            score2_game1 = st.number_input(label=f"Score for {team2_names} (Game 1)", key="score_gold_2_game1", min_value=0)

        # Game 2
        with col2:
            st.subheader("Game 2")
            score1_game2 = st.number_input(label=f"Score for {team1_names} (Game 2)", key="score_gold_1_game2", min_value=0)
            score2_game2 = st.number_input(label=f"Score for {team2_names} (Game 2)", key="score_gold_2_game2", min_value=0)

        # Game 3 (if required)
        st.subheader("Game 3 (if required)")
        score1_game3 = st.number_input(label=f"Score for {team1_names} (Game 3)", key="score_gold_1_game3", min_value=0)
        score2_game3 = st.number_input(label=f"Score for {team2_names} (Game 3)", key="score_gold_2_game3", min_value=0)
