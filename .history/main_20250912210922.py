...
def playoffs():
    if "playoff_teams" in st.session_state and all(len(team) == 2 for team in st.session_state.playoff_teams.values()):
        def playoff_match_form(round_name, team1, team2, team3, team4):
            with st.form(key=f"playoff_form_{round_name}"):
            st.header(round_name)
            col1, col2 = st.columns(2)
            ...
            with col1:
                ...
                team2_names = " + ".join(get_player_name(pid) for pid in team2)
                st.write(f"{team1_names} vs. {team2_names}")
                score1 = st.number_input(label=f"Score for {team1_names}", key=f"playoff_score_{round_name}_1", min_value=0)
                score2 = st.number_input(label=f"Score for {team2_names}", key=f"playoff_score_{round_name}_2", min_value=0)

            with col2:
                team3_names = " + ".join(get_player_name(pid) for pid in team3)
                team4_names = " + ".join(get_player_name(pid) for pid in team4)
                st.write(f"{team3_names} vs. {team4_names}")
                score3 = st.number_input(label=f"Score for {team3_names}", key=f"playoff_score_{round_name}_3", min_value=0)
                score4 = st.number_input(label=f"Score for {team4_names}", key=f"playoff_score_{round_name}_4", min_value=0)

            submitted = st.form_submit_button(label="Submit")
            if submitted:
                # Initialize playoff scores if not already present
                ...
                winner1 = team2
                st.session_state.playoff_scores[f"{round_name}_match1"] = {
                "team1": team1,
                "team2": team2,
                "score1": score1,
                "score2": score2,
                ...
                winner2 = team4
                st.session_state.playoff_scores[f"{round_name}_match2"] = {
                "team3": team3,
                "team4": team4,
                "score3": score3,
                "score4": score4,
                ...
                st.session_state.playoff_progress = f"{round_name} scores updated"

        playoff_match_form(
            round_name="Playoffs: Semifinals",
