...
with st.form(key="player_names_input"):
    ...
    st.form_submit_button(label="Submit",key='player_name_form_button',on_click=pop_p_names_dict(p_names))

print("session state end: ", st.session_state)

if "player_name_form_button" in st.session_state and st.session_state.player_name_form_button == True:
    st.write(st.session_state.player_name_dict)