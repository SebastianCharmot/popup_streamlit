...
def draft_playoff_teams():
    ...
    for team in range(1, 5):
        if len(st.session_state.playoff_teams[team]) < 2:
            ...
            if st.button(f"Add to Team {team}", key=f"add_team_{team}_player_{len(st.session_state.playoff_teams[team]) + 1}"):
                selected_pid = next(
                ...
                # Force rerun to update dropdowns and hide completed team's dropdown
st.experimental_set_                st.query_params["updated"]="true"urn
